...
colors = ['dodgerblue', 'firebrick', 'limegreen',
          'darkorange', 'purple', 'yellow', 'cyan']
...
def drawEdge(edge, verts, ind, mult, ax, scale_max=None, max_thickness=10,
    ...
    ax.plot([vert1[0], hp[0]], [vert1[1], hp[1]],
            color=col1, linewidth=lw, alpha=transparency)
    ax.plot([hp[0], vert2[0]], [hp[1], vert2[1]],
            col2, linewidth=lw, alpha=transparency)
#     if show_val:
...
def leiwandPlot(graph, name='graph'):
    ...
    for uc_edge in edge_dict.keys():
        ...
        for ii, coloring in enumerate(edge_dict[uc_edge]):
            ...
            if loop:
                loose = 10 + 5 * ii
                data.append([weight, str(edge[0]), edge[2],
                            str(edge[1]), edge[3], loose])
            else:
                bend = -22.5 + (ii + 0.5) * 45 / mult
                data.append([weight, str(edge[0]), edge[2],
                            str(edge[1]), edge[3], bend])
...
def plotFromFile(filename, number_nodes=True, outfile=""):
    ...
    graphPlotOld(sol_dict['graph'], scaled_weights=True,
                 number_nodes=number_nodes, filename=outfile)
...
def convert_bools_to_ints(input_list):
    converted_list = [int(item) if isinstance(item, bool)
...
def calculate_b(nb, x):
    b = (np.array(range(nb))-0.5*(nb-1)) * x / \
        (np.max(np.arange(1, nb+1))-0.5*(nb-1))
    return b
...
def plot_diamond(ax, center_x, center_y, diamond_width, diamond_height, zorder):
    ...
    diamond_x = [center_x, center_x + diamond_width / 2,
                 center_x, center_x - diamond_width / 2, center_x]
...
def plot_triangle(ax, center, side_length, zorder, linewidth):
    ...
    y = [center_y - (3**0.5)*side_length/6, center_y - (3**0.5)
         * side_length/6, center_y + (2*(3**0.5)*side_length)/6]
            ...
            for i, vertex in enumerate(vertices):
                if i in ancilla:
                    if i in single_emitters:
                        plot_triangle(ax, vertex, 2.5*r, 10,
                                      linewidth=linewidth)
                    else:
                        x = vertex[0]-r
                        y = vertex[1]-r
...
def Plot_Edges(ax, V1, V2, colors, side_length, max_len, max_thickness=5,
    ...
    if V1 == V2:
        ...
        for ed in range(len(b)):
            xi = h + a * np.cos(theta) * np.cos(t1) - \
                b[ed] * np.sin(theta) * np.sin(t1)
            yi = k + a * np.sin(theta) * np.cos(t1) + \
                b[ed] * np.cos(theta) * np.sin(t1)
            xj = h + a * np.cos(theta) * np.cos(t2) - \
                b[ed] * np.sin(theta) * np.sin(t2)
            yj = k + a * np.sin(theta) * np.cos(t2) + \
                b[ed] * np.cos(theta) * np.sin(t2)
            w = colors[ed][1]
            ax.plot(xi, yi, color=colors[ed][0][0], linewidth=line_width, alpha=transparency(
                w), zorder=9)
            ax.plot(xj, yj, color=colors[ed][0][1], linewidth=line_width, alpha=transparency(
                w), zorder=9)
            if w < 0:
...
def Plot_PBS(ax, X, Y, width, height, color1, color2):
    ...
    ax.plot([X, X-d0], [Y+2*d0, Y+3*d0],
            zorder=20, color=color2, linestyle=':')
...
def Plot_SPDC(ax, X, Y, width, height, color1, color2, W):
    ...
    ax.add_patch(Rectangle((X, Y), width/2, height,
                 fc=color1, ec='none', alpha=alpha))
    ax.add_patch(Rectangle((X+width/2, Y), width/2, height,
                 fc=color2, ec='none', alpha=alpha))
    ax.add_patch(Rectangle((X, Y), width, height,
                 fc='none', ec='black', zorder=10))
    d0 = width/10
    d1 = Y+height
...
def Plot_Absorber(ax, X, Y,  width, height):
    ax.add_patch(Rectangle((X, Y), width, height, fc='k',
                 ec='r', zorder=10, joinstyle='bevel', lw=2))
...
def Plot_Vline(ax, YMIN, YMAX, X, color):
    ax.vlines(X, ymin=YMIN, ymax=YMAX, colors=color, zorder=8)
...
def Plot_Detector(ax, X, Y, leng, step, radius):
    ...
    for ii in range(len(pos)):
        ax.add_patch(Wedge((pos[ii], Y), radius, 0,
                     180, fc='k', ec='k', zorder=10))
        ax.add_patch(Rectangle((pos[ii]-1.2*radius, Y-radius/2),
                     2.4*radius, radius/2, fc='k', ec='k', zorder=12))
        Plot_Connection_Line(
            ax, [pos[ii], pos[ii]-radius], [Y+radius, Y+2.5*radius])


def Plot_Crystal(ax, X, Y, color, width, height, W):  # for path identity
    ax.add_patch(Rectangle((X, Y), width, height,
                 fc='none', ec='black', zorder=6))
    row = len(color)
    column = 2
...
def Plot_Sorter(ax, X, Y, leng, step, width, height, color):
    ...
    for p in range(len(pos)):
        ax.add_patch(Circle((pos[p] + width/2, Y+height/2),
                     width/2, fc=color[p], ec='k', zorder=15))
        ax.add_patch(Rectangle((pos[p], Y), width,
                     height, fc='lightgray', ec='k', zorder=12))
...
def layer0fcrystal(crystal_lst, Numphoton):
    ...
    while len(res) > 0:
        r = res[0]
        layer0.append(r)
        res = [[ele for j, ele in enumerate(
            sub) if ele not in r] for i, sub in enumerate(res)]
        for item in res:
            if 0 < len(item) < ll:
...
def PlotPathIdentity(graph,  filename="", width=0.1, figsize=(8, 8),
    ...
    if count > 0:
        ...
        Remove_Multiedge = list(tuple(GraphEdgesAlphabet)
                                for GraphEdgesAlphabet, _ in itertools.groupby(GraphEdgesAlphabet))
        ...
        if len(th.findPerfectMatchings(Remove_Multiedge)) == 0:
            ...
            raise ValueError(
                "It appears that there is no perfect matching in the graph.")
        ...
        if len(NotInPM) > 0:
            Layers2 = union([NotInPM[i:i+1]
                            for i in range(0, len(NotInPM), 1)])
        ...
        w_spdc = Get_Color_Weight_Crystals(
            GraphEdgesAlphabet, Numphoton, Graphweight, Layers)
        ...
        for ii in range(len(PosxSpdc)):
            ...
            for jj in range(len(PosxSpdc[ii])):
                Plot_Crystal(ax, PosxSpdc[ii][jj], PosySpdc[ii][jj],
                             color_spdc[ii][jj], width, height, w_spdc[ii][jj])
        ...
        ax.set_aspect(1)
        ax.axis('off')
        experiment = fig.savefig(filename + ".png", bbox_inches='tight')
    return experiment


def PlotBulkOpticsPathEncoding(graph, task='PathEncoding', filename='', width=0.1, figsize=(16, 16), fontsize=16, colors=colors, Paths=Paths):
    graph = convert_to_fancy_graph(graph)
    GraphEdgesAlphabet = GetGraphColorEdge(graph, 0, Paths)
    GraphEdgesColor = GetGraphColorEdge(graph, 1, colors)
    Graphweight = convert_bools_to_ints(graph.weights)
    ...
    if len(Pos) > 0:
        ...
        for ii in range(len(PosX1)):
            ...
            Plot_Absorber(ax,  x-2*d0-height/4, y+2.5 *
        ...
        for ii in range(len(GET_DUBL)):
            ...
            for jj in range(len(x)):
                x[jj] = P2[x[jj]]
        ColX0 = [AllColor[PosX0fpath.index(pos)]
                 for pos in PosX0fpath if pos in PosX0]
        ...
        for ii in range(len(GET_DUBL)):
            ...
            xh2.append(x[1])
            yh2.append(y[1])
            color = [AllColor[PosX0fpath.index(pos)]
                     for pos in PosX0fpath if pos in x[1]]
            col1.append(color)
        ...
        for ii in range(len(xh1)):
            for jj in range(len(xh1[ii])-1):
                Plot_Hline(ax, xh1[ii][jj], xh2[ii][jj]-2 *
                           d0, yh1[ii][jj]+2*d0, col1[ii][jj])
                Plot_Vline(ax, yh1[ii][jj]+2*d0, yh2[ii]
                           [jj]+d0, xh2[ii][jj]-2*d0, col1[ii][jj])
        ...
        for pos in range(len(ynab2)):
            Plot_Vline(ax, ynab2[pos]-3*height+2*d0,
                       ynab2[pos], xnab2[pos], ColX0[pos])

        PosX_NAB = xnab1 + xnab2
        PosY_NAB = ynab1 + ynab2
        Path1nab = pathnab1
        Path2nab = [AllPath[PosX0fpath.index(pos)]
                    for pos in PosX0fpath if pos in PosX0]
        ...
        Color_R = [AllColor[PosX0fpath.index(pos)]
                   for pos in PosX0fpath if pos in PoSX_R]
        Path_R = [AllPath[PosX0fpath.index(pos)]
                  for pos in PosX0fpath if pos in PoSX_R]
    ...
    for sp in range(len(single_path)):
        ...
        Write_Label(ax, single_path[sp][0], single_path[sp]
                    [1]+height/2, single_path[sp][3][0], fontsize)
    ...
    try:
        if len(get_to_posX) > 0:
            ...
            for index in range(len(get_to_posX)):
                ...
                x = XD[index]
                y = YD[index]
                XtoD[index][1] = Pos_Element(
                    XD[index], height, len(XtoD[index][1]))
                xd = XtoD[index][1]
                yd = YtoD[index]
            ...
            for px in range(len(get_to_posX)):
                ...
                if task == 'BulkOptics':
                    Plot_Sorter(
                        ax, XD[px]-height/4, YD[px], len(get_to_posX[px][1]), height, height/2, height, c)
                    Plot_Multi_Color_Line(
                        ax, pathx_sort[px], YD[px]+height, 2*height, c, len(c)+1, height/3)
                    Write_Label(ax, pathx_sort[px],
                                YD[px]+3.5*height, tt,  fontsize)
                elif task == 'PathEncoding':
                    Plot_Detector(ax, XD[px], YD[px], len(
                        get_to_posX[px][1]), height, height/3)
...
def graphPlot(graph, type_photons=None, DistanceOfVertices=0.1, filename='',
    ...
    if show:
        ...
        pass
    if filename:
        fig = fig.savefig(filename + ".png", bbox_inches='tight')
    return fig
